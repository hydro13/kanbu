...
export async function handleListGitHubPRs(args: unknown) {
  ...
  result.prs.forEach((pr, index) => {
    ...
    lines.push(
      `${index + 1}. ${stateIcon} #${pr.prNumber}: ${truncate(pr.title, 60)}${linkedTask}`
    );
...
export async function handleSyncGitHubIssues(args: unknown) {
  ...
  const lines: string[] = ['Issue sync started!', '', result.message];
...
export async function handleCreateGitHubBranch(args: unknown) {
  ...
  const result = await client.call<{
    ...
    branchName: string;
    branchUrl?: string;
  }>(config.kanbuUrl, config.token, 'github.createBranch', { taskId, customBranchName }, 'POST');
...
export async function handleLinkPRToTask(args: unknown) {
  ...
  const result = await client.call<{
    ...
    linked: boolean;
    prUrl: string;
  }>(config.kanbuUrl, config.token, 'github.linkPRToTask', { prId, taskId }, 'POST');
