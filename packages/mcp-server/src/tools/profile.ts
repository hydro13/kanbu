...
export async function handleGetTimeTracking(args: unknown) {
  ...
  try {
    ...
    for (const proj of data.byProject) {
      const progress =
        proj.timeEstimated > 0 ? Math.round((proj.timeSpent / proj.timeEstimated) * 100) : 0;
      lines.push(
        `  ${proj.projectName}: ${formatHours(proj.timeSpent)} / ${formatHours(proj.timeEstimated)} (${progress}%)`
      );
    }
    ...
    return error(
      `Failed to get time tracking: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleGetLogins(args: unknown) {
  ...
  try {
    ...
    const lines: string[] = [`Login History (${data.total} total)`, ''];
...
export async function handleGetPasswordHistory(args: unknown) {
  ...
  try {
    const data = await client.call<{
      resets: PasswordResetEntry[];
      total: number;
      hasMore: boolean;
    }>(config.kanbuUrl, config.token, 'user.getPasswordResets', {
    ...
    const lines: string[] = [`Password Reset History (${data.total} total)`, ''];
    ...
    return error(
      `Failed to get password history: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleGetMetadata(_args: unknown) {
  ...
  try {
    ...
    const lines: string[] = [`User Metadata (${metadata.length} entries)`, ''];
...
export async function handleRemoveAvatar(_args: unknown) {
  ...
  try {
    await client.call<{ success: boolean }>(config.kanbuUrl, config.token, 'user.removeAvatar', {});

    return success('Avatar removed successfully.');
  } catch (err) {
    return error(
      `Failed to remove avatar: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleGet2FAStatus(_args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to get 2FA status: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleDisableOwn2FA(args: unknown) {
  ...
  try {
    await client.call<{ success: boolean }>(config.kanbuUrl, config.token, 'user.disable2FA', {
...
export async function handleGetPublicAccess(_args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to get public access: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleEnablePublicAccess(_args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to enable public access: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleDisablePublicAccess(_args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to disable public access: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleRegeneratePublicToken(_args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to regenerate token: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleGetNotificationSettings(_args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to get notification settings: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleUpdateNotificationSettings(args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to update notification settings: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleListExternalAccounts(_args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to list external accounts: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleUnlinkExternalAccount(args: unknown) {
  ...
  try {
    await client.call<{ success: boolean }>(config.kanbuUrl, config.token, 'user.unlinkAccount', {
    ...
    return error(
      `Failed to unlink account: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleListApiTokens(_args: unknown) {
  ...
  try {
    ...
    const lines: string[] = [`API Tokens (${tokens.length})`, ''];
    ...
    return error(
      `Failed to list API tokens: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleCreateApiToken(args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to create API token: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleRevokeApiToken(args: unknown) {
  ...
  try {
    await client.call<{ success: boolean }>(config.kanbuUrl, config.token, 'apiKey.revoke', {
      keyId: input.keyId,
    });
    ...
    return error(
      `Failed to revoke API token: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleGetApiPermissions(_args: unknown) {
  ...
  try {
    const permissions = await client.call<
      Array<{ value: string; label: string; description: string }>
    >(config.kanbuUrl, config.token, 'apiKey.getPermissions', {});

    const lines: string[] = ['Available API Permissions', ''];
    ...
    return error(
      `Failed to get API permissions: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleListAiBindings(_args: unknown) {
  ...
  try {
    ...
    const lines: string[] = [`AI Assistant Bindings (${bindings.length})`, ''];
    ...
    return error(
      `Failed to list AI bindings: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleRevokeAiBinding(args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to revoke AI binding: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleGetHourlyRate(_args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to get hourly rate: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleSetHourlyRate(args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to set hourly rate: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleDeleteMetadata(args: unknown) {
  ...
  try {
    await client.call<{ success: boolean }>(config.kanbuUrl, config.token, 'user.deleteMetadata', {
      key: input.key,
    });
    ...
    return error(
      `Failed to delete metadata: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleRevokeSession(args: unknown) {
  ...
  try {
    await client.call<{ success: boolean }>(config.kanbuUrl, config.token, 'user.revokeSession', {
...
export async function handleRevokeAllSessions(_args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to revoke all sessions: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
