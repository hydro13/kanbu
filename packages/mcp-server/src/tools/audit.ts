...
export const auditToolDefinitions = [
  {
    ...
    description: 'List audit logs with filtering and pagination. Domain Admins see all, Workspace Admins see their workspace only.',
    inputSchema: {
      type: 'object',
      properties: {
        category: { type: 'string', enum: ['ACL', 'GROUP', 'USER', 'WORKSPACE', 'SETTINGS'], description: 'Filter by category' },
...
export async function handleListAuditLogs(args: unknown) {
  ...
  try {
    ...
    if (result.hasMore) {
      lines.push(`Showing ${result.logs.length} of ${result.total}. Use offset=${result.offset + result.limit} for more.`)
    }

    return success(lines.join('\n'))
  } catch (err) {
    return error(`Failed to list audit logs: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleGetAuditLog(args: unknown) {
  ...
  try {
    const log = await client.call<AuditLogEntry>(
      config.kanbuUrl,
      config.token,
      'auditLog.get',
      { id: input.id }
    ...
    return error(`Failed to get audit log: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleAuditStats(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to get audit stats: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleExportAuditLogs(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to export audit logs: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleGetAuditCategories(_args: unknown) {
  ...
  try {
    ...
    return error(`Failed to get audit categories: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
