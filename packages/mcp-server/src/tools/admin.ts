...
export async function handleListUsers(args: unknown) {
  ...
  try {
    ...
    for (const user of result.users) {
      ...
      lines.push(
        `${roleEmoji(user.role)} #${user.id} ${user.name} (@${user.username})${domainAdmin}${status}`
      );
      lines.push(`   Email: ${user.email}`);
      lines.push(
        `   Last login: ${formatDate(user.lastLoginAt)} | Workspaces: ${user.workspaceCount}`
      );
  ...
  try {
    ...
    for (const login of result.logins) {
      ...
      if (login.userAgent) {
        ...
        const ua =
          login.userAgent.length > 60 ? login.userAgent.slice(0, 60) + '...' : login.userAgent;
        lines.push(`   ${ua}`);
      }
    }
    ...
    return error(
      `Failed to get user logins: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleDeleteUser(args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to deactivate user: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleReactivateUser(args: unknown) {
  ...
  try {
    ...
    return error(
      `Failed to reactivate user: ${err instanceof Error ? err.message : 'Unknown error'}`
    );
  }
}
...
export async function handleResetPassword(args: unknown) {
  ...
  try {
    ...
    return success(
      `Password reset successfully for user #${input.userId}. ${result.message ?? ''}`
    );
  } catch (err) {
    return error(
...
export async function handleRevokeSessions(args: unknown) {
  ...
  try {
    ...
    return success(
      `All sessions revoked for user #${input.userId}. ${result.count ?? 0} session(s) terminated.`
    );
  } catch (err) {
    return error(
