...
interface CalculateEffectiveResponse {
  ...
  directEntries: Array<{ id: number; permissions: number; presetName: string | null; deny: boolean; inheritToChildren: boolean }>
  groupEntries: Array<{ groupId: number; groupName: string; permissions: number; presetName: string | null; deny: boolean }>
...
interface SimulateChangeResponse {
  ...
  summary: { new: number; upgraded: number; downgraded: number; unchanged: number; removed: number }
  warnings: string[]
}
...
export const BulkGrantSchema = z.object({
  ...
  principals: z.array(z.object({
    ...
    id: z.number(),
  })).min(1).max(100).describe('Principals to grant permissions to'),
...
export const CopyPermissionsSchema = z.object({
  ...
  })).min(1).max(50).describe('Target resources'),
  overwrite: z.boolean().optional().describe('Overwrite existing entries'),
})

export const ApplyTemplateSchema = z.object({
  templateName: z.enum(['read_only', 'contributor', 'editor', 'full_control']).describe('Template name'),
  resourceType: ResourceTypeSchema.describe('Resource type'),
  resourceId: z.number().nullable().describe('Resource ID (null for root)'),
  principals: z.array(z.object({
    type: PrincipalTypeSchema,
    id: z.number(),
  })).min(1).max(100).describe('Principals to apply template to'),
  inheritToChildren: z.boolean().optional().describe('Inherit to children'),
})
...
export const SimulateChangeSchema = z.object({
  ...
  principals: z.array(z.object({
    type: PrincipalTypeSchema,
    id: z.number(),
  })).min(1).max(100).describe('Principals'),
  permissions: z.number().optional().describe('Permission bitmask (for grant/deny)'),
  templateName: z.enum(['read_only', 'contributor', 'editor', 'full_control']).optional().describe('Template name (for template action)'),
})
...
export const aclToolDefinitions = [
  // Query Tools
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        resourceType: { type: 'string', enum: ['root', 'system', 'dashboard', 'workspace', 'project', 'feature', 'admin', 'profile'], description: 'Resource type' },
  ...
  },
  {
    ...
    description: 'Check effective permissions for a user on a resource. Shows the permission calculation breakdown.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Get your effective permissions on a resource. Quick way to check what you can do.',
    inputSchema: {
      type: 'object',
      properties: {
        resourceType: { type: 'string', enum: ['root', 'system', 'dashboard', 'workspace', 'project', 'feature', 'admin', 'profile'], description: 'Resource type' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        principalTypes: { type: 'array', items: { type: 'string' }, description: 'Filter by principal types (user, group)' },
  ...
  },
  {
    ...
    description: 'Calculate effective permissions with detailed breakdown. Shows direct, group, and inherited entries.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  // Management Tools
  {
    ...
    description: 'Grant permissions to a user or group on a resource. Requires P (manage permissions) on the resource.',
    inputSchema: {
      type: 'object',
      properties: {
        resourceType: { type: 'string', enum: ['root', 'system', 'dashboard', 'workspace', 'project', 'feature', 'admin', 'profile'], description: 'Resource type' },
        ...
        permissions: { type: 'number', description: 'Permission bitmask (1=R, 2=W, 4=X, 8=D, 16=P, or presets: 1=Read, 7=Contributor, 15=Editor, 31=Full)' },
        inheritToChildren: { type: 'boolean', description: 'Inherit to child resources (default true)' },
      },
  ...
  },
  {
    ...
    description: 'Deny specific permissions to a user or group. Deny entries override grants (like NTFS).',
    inputSchema: {
      type: 'object',
      properties: {
        resourceType: { type: 'string', enum: ['root', 'system', 'dashboard', 'workspace', 'project', 'feature', 'admin', 'profile'], description: 'Resource type' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        resourceType: { type: 'string', enum: ['root', 'system', 'dashboard', 'workspace', 'project', 'feature', 'admin', 'profile'], description: 'Resource type' },
        resourceId: { type: ['number', 'null'], description: 'Resource ID (null for root)' },
        principals: { type: 'array', items: { type: 'object', properties: { type: { type: 'string' }, id: { type: 'number' } } }, description: 'Principals [{type, id}, ...]' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        sourceResourceType: { type: 'string', enum: ['root', 'system', 'dashboard', 'workspace', 'project', 'feature', 'admin', 'profile'], description: 'Source resource type' },
        sourceResourceId: { type: ['number', 'null'], description: 'Source resource ID' },
        targetResources: { type: 'array', items: { type: 'object', properties: { type: { type: 'string' }, id: { type: ['number', 'null'] } } }, description: 'Target resources [{type, id}, ...]' },
  ...
  },
  {
    ...
    description: 'Apply a permission template (read_only, contributor, editor, full_control) to multiple principals.',
    inputSchema: {
      type: 'object',
      properties: {
        templateName: { type: 'string', enum: ['read_only', 'contributor', 'editor', 'full_control'], description: 'Template name' },
        resourceType: { type: 'string', enum: ['root', 'system', 'dashboard', 'workspace', 'project', 'feature', 'admin', 'profile'], description: 'Resource type' },
        resourceId: { type: ['number', 'null'], description: 'Resource ID (null for root)' },
        principals: { type: 'array', items: { type: 'object', properties: { type: { type: 'string' }, id: { type: 'number' } } }, description: 'Principals [{type, id}, ...]' },
        inheritToChildren: { type: 'boolean', description: 'Inherit to children' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        action: { type: 'string', enum: ['grant', 'deny', 'revoke', 'template'], description: 'Action to simulate' },
        resourceType: { type: 'string', enum: ['root', 'system', 'dashboard', 'workspace', 'project', 'feature', 'admin', 'profile'], description: 'Resource type' },
        resourceId: { type: ['number', 'null'], description: 'Resource ID' },
        principals: { type: 'array', items: { type: 'object', properties: { type: { type: 'string' }, id: { type: 'number' } } }, description: 'Principals' },
        permissions: { type: 'number', description: 'Permission bitmask (for grant/deny)' },
        templateName: { type: 'string', enum: ['read_only', 'contributor', 'editor', 'full_control'], description: 'Template name (for template action)' },
      },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        resourceType: { type: 'string', enum: ['root', 'system', 'dashboard', 'workspace', 'project', 'feature', 'admin', 'profile'], description: 'Resource type filter (optional)' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        mode: { type: 'string', enum: ['skip', 'overwrite', 'merge'], description: 'Import mode: skip existing, overwrite, or merge' },
      },
...
export async function handleListAcl(args: unknown) {
  ...
  try {
    ...
    if (entries.length === 0) {
      return success(`No ACL entries for ${input.resourceType}${input.resourceId ? ` #${input.resourceId}` : ' (root)'}.`)
    }
    ...
    if (denyEntries.length > 0) {
      ...
      for (const entry of denyEntries) {
        lines.push(`[DENY] ${entry.principalType}:${entry.principalDisplayName} ${permissionBitsToString(entry.permissions)}`)
        lines.push(`   Preset: ${entry.presetName ?? 'Custom'} | Inherit: ${entry.inheritToChildren ? 'Yes' : 'No'}`)
      }
      lines.push('')
    }
    ...
    if (allowEntries.length > 0) {
      ...
      for (const entry of allowEntries) {
        lines.push(`${entry.principalType}:${entry.principalDisplayName} ${permissionBitsToString(entry.permissions)}`)
        lines.push(`   ID: ${entry.id} | Preset: ${entry.presetName ?? 'Custom'} | Inherit: ${entry.inheritToChildren ? 'Yes' : 'No'}`)
      }
    }

    return success(lines.join('\n'))
  } catch (err) {
    return error(`Failed to list ACL entries: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleCheckPermission(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to check permission: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleMyPermission(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to get your permissions: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleGetPrincipals(args: unknown) {
  ...
  try {
    ...
    for (const group of result.groups.slice(0, 20)) {
      lines.push(`  #${group.id} ${group.displayName} [${group.groupType}] (${group.memberCount} members)`)
    }
    ...
    return error(`Failed to get principals: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleGetResources(_args: unknown) {
  ...
  try {
    ...
    for (const proj of result.projects.slice(0, 10)) {
      lines.push(`  #${proj.id} ${proj.name} (${proj.identifier}) - Workspace: ${proj.workspaceName}`)
    }
    ...
    return error(`Failed to get resources: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
  ...
  try {
    ...
    for (const principal of result.principals) {
      ...
      for (const cell of cells) {
        const resource = result.resources.find(r => r.type === cell.resourceType && r.id === cell.resourceId)
        const denied = cell.isDenied ? ' [DENIED]' : ''
        const inherited = !cell.isDirect ? ' (inherited)' : ''
        lines.push(`  ${resource?.path ?? cell.resourceType}: ${permissionBitsToString(cell.effectivePermissions)}${denied}${inherited}`)
      }
      lines.push('')
    }
    ...
    return error(`Failed to get permission matrix: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
  ...
  try {
    ...
    if (result.directEntries.length > 0) {
      ...
      for (const entry of result.directEntries) {
        ...
        lines.push(`  ${deny}${permissionBitsToString(entry.permissions)} (${entry.presetName ?? 'Custom'})`)
      }
      lines.push('')
    }
    ...
    if (result.inheritedEntries.length > 0) {
      ...
      for (const entry of result.inheritedEntries) {
        ...
        lines.push(`  ${deny}From ${entry.fromResourceName}: ${permissionBitsToString(entry.permissions)}${via}`)
      }
      lines.push('')
    }
    ...
    return error(`Failed to calculate effective permissions: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleGrantPermission(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to grant permission: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleDenyPermission(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to deny permission: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleRevokePermission(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to revoke permission: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleUpdateAcl(args: unknown) {
  ...
  try {
    ...
    return success(`ACL entry #${input.id} updated to ${permissionBitsToString(input.permissions)}.`)
  } catch (err) {
    return error(`Failed to update ACL entry: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleDeleteAcl(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to delete ACL entry: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleCopyPermissions(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to copy permissions: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleApplyTemplate(args: unknown) {
  ...
  try {
    ...
    return error(`Failed to apply template: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
...
export async function handleSimulateChange(args: unknown) {
  ...
  try {
    ...
    const lines: string[] = [
      `What-If Simulation: ${input.action}`,
      '',
      '== Changes ==',
    ]
    ...
    return error(`Failed to simulate change: ${err instanceof Error ? err.message : 'Unknown error'}`)
  }
}
