...
export async function handleListTasks(args: unknown) {
  ...
  const tasks = await client.call<Task[]>(config.kanbuUrl, config.token, 'task.list', {
...
export async function handleGetTask(args: unknown) {
  ...
  const task = await client.call<TaskWithDetails>(config.kanbuUrl, config.token, 'task.get', {
    taskId: id,
  });

  const columnInfo = task.column ? `${task.column.title} (ID: ${task.column.id})` : 'Unknown';
...
export async function handleMoveTask(args: unknown) {
  ...
  const beforeTask = await client.call<Task>(config.kanbuUrl, config.token, 'task.get', {
    taskId: input.id,
  });
  ...
  const afterTask = await client.call<Task>(config.kanbuUrl, config.token, 'task.get', {
    taskId: id,
  });
...
export async function handleMyTasks(args: unknown) {
  ...
  const tasks = await client.call<Task[]>(config.kanbuUrl, config.token, 'task.getAssignedToMe', {
