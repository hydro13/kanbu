...
const ACL_PRESETS = {
  ...
  EDITOR: ACL_PERMISSIONS.READ | ACL_PERMISSIONS.WRITE | ACL_PERMISSIONS.EXECUTE | ACL_PERMISSIONS.DELETE,
  FULL_CONTROL: ACL_PERMISSIONS.READ | ACL_PERMISSIONS.WRITE | ACL_PERMISSIONS.EXECUTE | ACL_PERMISSIONS.DELETE | ACL_PERMISSIONS.PERMISSIONS,
}
...
async function migrateProjectMembers(): Promise<number> {
  ...
  for (const pm of projectMembers) {
    ...
    if (!existing) {
      ...
      console.log(`  ✓ ${pm.user.username} -> ${pm.project.identifier} (${pm.role} = ${permissions})`)
      migrated++
    } else {
  ...
  for (const gp of groupPermissions) {
    ...
    switch (gp.permission.toLowerCase()) {
      ...
      default:
        ...
        if (gp.permission.includes('execute') || gp.permission.includes('create')) permissions |= ACL_PERMISSIONS.EXECUTE
        if (gp.permission.includes('delete')) permissions |= ACL_PERMISSIONS.DELETE
        if (gp.permission.includes('permission') || gp.permission.includes('admin')) permissions |= ACL_PERMISSIONS.PERMISSIONS
    }
    ...
    if (!existing) {
      ...
      console.log(`  ✓ Group:${gp.group.name} -> ${resourceType}:${resourceId ?? 'root'} (${gp.permission} = ${permissions})${denyStr}`)
      migrated++
    } else {
      console.log(`  - Group:${gp.group.name} -> ${resourceType}:${resourceId ?? 'root'} (already exists)`)
    }
  }
