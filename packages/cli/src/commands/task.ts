...
export const taskCommands = {
  ...
  async show(reference: string) {
    ...
    try {
      ...
      if (task.assignees.length > 0) {
        console.log(`  ${chalk.gray('Assignees:')}  ${task.assignees.map((a) => a.name).join(', ')}`)
      }
      ...
      if (task.pullRequests.length > 0) {
        console.log(`  ${chalk.gray('PRs:')}        ${task.pullRequests.map((pr) => `#${pr.prNumber} (${pr.state})`).join(', ')}`)
      }
  ...
  async start(reference: string, options: StartOptions) {
    ...
    try {
      ...
      spinner.succeed(
        chalk.green(`Started working on ${chalk.bold(reference)}`)
      )
  ...
  async create(options: CreateOptions) {
    ...
    const projectSlug = options.project || config.get('defaultProject')
    if (!projectSlug) {
      console.log(chalk.red('Project required. Use --project or set default with: kanbu set-project <slug>'))
      return
    }
