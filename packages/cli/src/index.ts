...
program.command('logout').description('Logout from Kanbu').action(authCommands.logout);

program.command('whoami').description('Show current user').action(authCommands.whoami);
...
taskCmd.command('show <reference>').description('Show task details').action(taskCommands.show);
...
taskCmd.command('done <reference>').description('Mark task as done').action(taskCommands.done);
...
prCmd.command('status').description('Show PR status for current branch').action(prCommands.status);

prCmd.command('link <task>').description('Link current PR to a task').action(prCommands.link);
