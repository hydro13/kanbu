...
import { WikiNodeEmbeddingService } from '../apps/api/src/lib/ai/wiki/WikiNodeEmbeddingService';
...
const NODE_TYPES = nodeTypesArg ? nodeTypesArg.split(',').map((t) => t.trim()) : DEFAULT_NODE_TYPES;
...
async function duplicateEdgeExists(
  ...
  try {
    ...
    return (
      (rows[0] as boolean[])![0] === true || (reverseResult[1] as boolean[][])?.[0]?.[0] === true
    );
  } catch (error) {
    if (VERBOSE) {
...
async function main() {
  ...
  // Build groupId
  const groupId = PROJECT_ID ? `wiki-proj-${PROJECT_ID}` : `wiki-ws-${WORKSPACE_ID}`;
  ...
  console.log('');
  console.log(
    `Mode:        ${DRY_RUN ? '\x1b[33mDRY RUN\x1b[0m (no edges created)' : '\x1b[32mAPPLY\x1b[0m (edges will be created)'}`
  );
  ...
  try {
    ...
    for (const [matchType, dups] of Object.entries(byMatchType)) {
      ...
      for (const dup of dups.slice(0, VERBOSE ? 50 : 10)) {
        ...
        console.log(
          `    ${srcName.padEnd(25)} ≈ ${tgtName.padEnd(25)} ${formatConfidence(dup.confidence)}`
        );
      }
    ...
    if (!DRY_RUN) {
      ...
      for (const dup of duplicates) {
        const exists = await duplicateEdgeExists(redis, dup.sourceNode.uuid, dup.targetNode.uuid);

        if (exists) {
          edgesSkipped++;
          if (VERBOSE) {
            console.log(
              `  ⏭️  Edge already exists: ${dup.sourceNode.name} → ${dup.targetNode.name}`
            );
          }
          continue;
        }
