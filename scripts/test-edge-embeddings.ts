...
async function main() {
  ...
  try {
    ...
    results.push(
      await runTest('Edge Embeddings Collection', async () => {
    ...
    results.push(
      await runTest('FalkorDB Edges Count', async () => {
    ...
    results.push(
      await runTest('Get Sample Edge', async () => {
        const result = (await redis.call(
          'GRAPH.QUERY',
          GRAPH_NAME,
          `
    ...
    results.push(
      await runTest('Edge Semantic Search', async () => {
        ...
        const searchResults = await edgeService.edgeSemanticSearch(context, searchQuery, {
          limit: 5,
          scoreThreshold: 0.3,
        });
    ...
    results.push(
      await runTest('Hybrid Semantic Search', async () => {
        const context: WikiContext = { workspaceId: WORKSPACE_ID };

        const searchQuery = sampleEdge?.targetName ?? 'wiki';
        const hybridResults = await edgeService.hybridSemanticSearch(context, searchQuery, {
          includePages: true,
          includeEdges: true,
          limit: 10,
          scoreThreshold: 0.3,
