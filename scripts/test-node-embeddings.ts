...
async function main() {
  ...
  try {
    ...
    results.push(await runTest('Node Embeddings Collection', async () => {
    ...
    results.push(await runTest('FalkorDB Entity Count', async () => {
    ...
    results.push(await runTest('Get Sample Entities', async () => {
      const result = await redis.call(
        'GRAPH.QUERY',
        GRAPH_NAME,
        `
    ...
    results.push(await runTest('Service Initialization', async () => {
    ...
    if (sampleEntities.length > 0) {
      results.push(await runTest('Store Test Embedding', async () => {
    ...
    results.push(await runTest('Entity Semantic Search', async () => {
      if (sampleEntities.length === 0) {
        return 'Skipped (no entities)'
      }

      const searchName = sampleEntities[0].name.split(' ')[0] // First word
      const searchResults = await nodeService.findSimilarEntities(
        context,
        searchName,
    ...
    results.push(await runTest('Exact Name Lookup', async () => {
      if (sampleEntities.length === 0) {
        return 'Skipped (no entities)'
      }

      const exactName = sampleEntities[0].name
      const exactResults = await nodeService.findByNormalizedName(
        context,
        exactName,
        { limit: 5 }
      )
    ...
    results.push(await runTest('Entity Resolution (Partial Name)', async () => {
      if (sampleEntities.length === 0) {
        return 'Skipped (no entities)'
      }
      ...
      const resolveResults = await nodeService.findSimilarEntities(
        context,
        firstName,
        {
    ...
    if (testNodeId) {
      results.push(await runTest('Cleanup Test Embedding', async () => {
