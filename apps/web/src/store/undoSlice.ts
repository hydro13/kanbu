...
const undoSlice = createSlice({
  ...
  reducers: {
    ...
    updateTaskSnapshot: (state, action: PayloadAction<{ taskId: number; newUpdatedAt: string }>) => {
    ...
    updateLastActionSnapshot: (state, action: PayloadAction<{ taskId: number; newUpdatedAt: string }>) => {
      let undoUpdated = 0
      let redoUpdated = 0
    ...
    updateLastRedoActionSnapshot: (state, action: PayloadAction<{ taskId: number; newUpdatedAt: string }>) => {
      let undoUpdated = 0
      let redoUpdated = 0
      // Update ALL actions for this task in BOTH stacks
      for (let i = 0; i < state.undoStack.length; i++) {
