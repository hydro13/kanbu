...
export function AiSystemsPage() {
  ...
  return (
    ...
    >
      ...
      {/* Loading state */}
      {isLoading && <div className="text-center py-12 text-gray-500">Loading AI providers...</div>}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex-1">
          ...
          {/* Scope info */}
          {provider.workspace && (
            <p className="text-xs text-gray-400">Workspace: {provider.workspace.name}</p>
          )}
          {provider.project && (
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {(Object.keys(PROVIDER_CONFIG) as ProviderType[]).map((type) => (
                ...
                >
                  <div className="font-medium text-foreground">{PROVIDER_CONFIG[type].label}</div>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({ ...prev, reasoningModel: e.target.value }))
                }
                className="w-full px-3 py-2 text-sm border border-input rounded-lg bg-background text-foreground focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
...
function EditProviderModal({ providerId, onClose, onSubmit, isLoading }: EditProviderModalProps) {
  const { data: provider, isLoading: loadingProvider } = trpc.aiProvider.get.useQuery({
    id: providerId,
  });
  ...
  const handleCapabilityToggle = (cap: Capability) => {
    ...
    setFormData((prev) => ({
      ...prev,
      capabilities: current.includes(cap) ? current.filter((c) => c !== cap) : [...current, cap],
    }));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.isActive !== undefined && formData.isActive !== provider.isActive)
      changes.isActive = formData.isActive;
    if (formData.priority !== undefined && formData.priority !== provider.priority)
      changes.priority = formData.priority;
    if (formData.capabilities) changes.capabilities = formData.capabilities;
    if (formData.baseUrl && formData.baseUrl !== provider.baseUrl)
      changes.baseUrl = formData.baseUrl;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-card rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div>
            <h2 className="text-lg font-semibold text-foreground">Edit Provider</h2>
            <span
              className={cn(
                'px-2 py-0.5 text-xs font-medium rounded mt-1 inline-block',
                config.color
              )}
            >
              {config.label}
