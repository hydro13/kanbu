...
export function ExternalAccounts() {
  ...
  const handleLinkClick = (provider: Provider) => {
    ...
    alert(
      `OAuth linking for ${providers[provider].name} requires server-side OAuth configuration. This feature will be available once OAuth credentials are configured.`
    );
  };
  ...
  const connectedCount = accounts
    ? [accounts.google.connected, accounts.github.connected, accounts.gitlab.connected].filter(
        Boolean
      ).length
    : 0;
        ...
        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {connectedCount === 0
                ? 'Link an account for easier sign-in'
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded ${isConnected ? 'bg-green-50 dark:bg-green-900/20' : 'bg-gray-100 dark:bg-gray-800'}`}
                  >
                    {config.icon}
                  </div>
