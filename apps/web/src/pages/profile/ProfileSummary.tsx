...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium ${colors[role as keyof typeof colors] || colors.MEMBER}`}
    >
      {role.toLowerCase()}
    </span>
  );
            ...
            <div className="space-y-2">
              ...
              {profile.workspaces.length > 3 && (
                <p className="text-xs text-muted-foreground">
                  +{profile.workspaces.length - 3} more
                </p>
              )}
            </div>
          ) : (
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Section title="Security">
          <InfoRow
            ...
            value={
              <StatusBadge
                active={profile.twofactorActivated}
                label={profile.twofactorActivated ? 'Enabled' : 'Disabled'}
              />
            }
          />
          <InfoRow
          ...
          <InfoRow
            ...
            value={
              <StatusBadge
                ...
                label={profile.notificationsEnabled ? 'On' : 'Off'}
              />
            }
          />
          <InfoRow
            label="Filter"
            value={notificationFilterLabels[profile.notificationFilter] || '-'}
          />
          {profile.hourlyRate && <InfoRow label="Hourly Rate" value={`€${profile.hourlyRate}`} />}
