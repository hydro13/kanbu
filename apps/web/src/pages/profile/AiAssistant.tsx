...
export function AiAssistant() {
  ...
  // Queries
  const { data: bindings, isLoading: bindingsLoading } = trpc.assistant.getBindings.useQuery(undefined, {
    refetchInterval: 5000, // Poll every 5 seconds when setup code is active
  })
  const { data: activeSetupCode, refetch: refetchSetupCode } = trpc.assistant.getActiveSetupCode.useQuery(undefined, {
  ...
  if (bindingsLoading) {
    return (
      <ProfileLayout title="AI Assistant" description="Connect Claude Code to manage projects on your behalf">
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Connect Claude Code to your Kanbu account. Claude will inherit your permissions
              and can create tasks, update projects, and more on your behalf.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {!activeSetupCode && (
              <Button
                ...
                className="mt-4"
              >
                {generateCode.isPending ? 'Generating...' : hasActiveBindings ? 'Connect Another Machine' : 'Generate Setup Code'}
              </Button>
            )}
          ...
          <CardContent>
            <div className="text-sm text-muted-foreground space-y-3">
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <strong>Audit Trail:</strong> All actions by Claude are logged with your name
                  and marked as "via Claude Code".
                </div>
              </div>
              <div className="flex gap-3">
                <div className="h-6 w-6 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center flex-shrink-0">
