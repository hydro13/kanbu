...
export function TimeTracking() {
  ...
  const efficiency =
    data.totalEstimated > 0 ? Math.round((data.totalSpent / data.totalEstimated) * 100) : 0;
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <div className="bg-card rounded-card border border-border p-3 text-center">
          <p className="text-xl font-bold text-blue-600 dark:text-blue-400">
            {formatHours(data.totalSpent)}
          </p>
          <p className="text-xs text-muted-foreground">Time Spent</p>
        </div>
        <div className="bg-card rounded-card border border-border p-3 text-center">
          <p className="text-xl font-bold text-gray-600 dark:text-gray-400">
            {formatHours(data.totalEstimated)}
          </p>
          <p className="text-xs text-muted-foreground">Estimated</p>
        </div>
        <div className="bg-card rounded-card border border-border p-3 text-center">
          <p
            className={`text-xl font-bold ${efficiency <= 100 ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'}`}
          >
            {efficiency}%
          </p>
