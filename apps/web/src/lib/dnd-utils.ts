 ...
 */
export function getColumnSwimlaneDroppableId(columnId: number, swimlaneId: number | null): string {
  if (swimlaneId === null) {
    return `column-${columnId}`;
  }
 ...
 */
export function calculateNewPosition(currentPositions: number[], targetIndex: number): number {
 ...
 */
export function wouldExceedWipLimit(currentCount: number, wipLimit: number): boolean {
 ...
 */
export function getDragStateClasses(isDragging: boolean, isOver: boolean): string {
