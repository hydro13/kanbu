  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
      <div className="flex items-center gap-3">
        ...
        />
        <span className="font-medium text-foreground">{contributor.login}</span>
      </div>
      ...
      {data.slice(-8).map((period, idx) => {
        ...
        return (
          <div key={idx} className="flex items-center gap-3">
            <span className="w-16 text-xs text-gray-500 dark:text-gray-400">{label}</span>
            <div className="flex-1 h-6 bg-gray-100 dark:bg-gray-700 rounded overflow-hidden">
              ...
              >
                {total > 0 && <span className="text-xs text-white font-medium">{total}</span>}
              </div>
            </div>
          </div>
        );
...
export function ProjectAnalyticsPanel({ projectId }: ProjectAnalyticsPanelProps) {
  ...
  const hasData =
    cycleTime.totalCompleted > 0 || reviewTime.totalPRsReviewed > 0 || contributors.length > 0;
    ...
    <div className="space-y-6">
      ...
      {cycleTime.totalCompleted > 0 && (
        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
          <h4 className="font-medium text-foreground mb-3">Cycle Time Breakdown</h4>
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">Slowest</div>
            </div>
          </div>
        </div>
      )}
      ...
      {throughput.length > 0 && (
        <div className="bg-card rounded-card border border-border p-4">
          <h4 className="font-medium text-foreground mb-4">Weekly Throughput</h4>
