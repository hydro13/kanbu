  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      const tasksInTarget = tasks
        .filter(
          (t) =>
            t.columnId === targetColumnId && t.swimlaneId === targetSwimlaneId && t.id !== taskId
        )
  ...
  return (
    ...
    >
      ...
      <DragOverlay dropAnimation={null}>
        {dragState.activeTask ? <DragOverlayCard task={dragState.activeTask} /> : null}
      </DragOverlay>
    </DndKitContext>
  );
}
...
function DragOverlayCard({ task }: { task: BoardTask }) {
  return (
    <div className="bg-background rounded-lg p-3 shadow-xl border-2 border-blue-400 dark:border-blue-500 opacity-90 w-64">
      ...
      {task.reference && (
        <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">{task.reference}</div>
      )}
    </div>
  );
}
