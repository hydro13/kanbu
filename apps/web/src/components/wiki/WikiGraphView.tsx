    ...
    <div className="w-80 border-l bg-background flex flex-col">
              ...
              <div className="flex justify-between">
                ...
                <span className="flex items-center gap-1">
                  <span
                    ...
                    style={{
                      backgroundColor: CLUSTER_COLORS[selectedNode.cluster % CLUSTER_COLORS.length],
                    }}
                  />
      ...
      {selectedNode.type === 'WikiPage' && selectedNode.slug && (
        <div className="p-4 border-t">
          <Button className="w-full" onClick={() => onNavigate(selectedNode.slug!)}>
...
function PathExplanation({ steps, onClose }: { steps: PathStep[]; onClose: () => void }) {
...
export function WikiGraphView({
  ...
  const nodeCirclesRef = useRef<d3.Selection<
    SVGCircleElement,
    GraphNode,
    SVGGElement,
    unknown
  ...
  const highlightedNodeIdsSet = useMemo(() => new Set(highlightedNodeIds), [highlightedNodeIds]);
  ...
  const isNodeHighlighted = useCallback(
    (node: GraphNode): boolean => {
      return (
        highlightedNodeIdsSet.has(node.id) ||
        (node.uuid !== undefined && highlightedNodeIdsSet.has(node.uuid))
      );
    },
  ...
  const processedData = useMemo(() => {
    if (!graphData)
      return {
        nodes: [],
        edges: [],
        nodeStats: {} as Record<GraphNode['type'], number>,
        totalNodesBeforeLimit: 0,
      };
  ...
  const handleSelectForPath = useCallback(
    (node: GraphNode) => {
      if (!pathStart) {
        setPathStart(node);
        setHoverCard(null);
  ...
  useEffect(() => {
    ...
    if (timelineMode && timeBounds.min && timeBounds.max) {
      ...
      const axis = d3
        ...
        .ticks(6)
        .tickFormat(
          d3.timeFormat('%Y-%m-%d') as unknown as (d: d3.NumberValue, i: number) => string
        );

      axisG.call(axis).selectAll('text').attr('fill', '#666').attr('font-size', '10px');

      axisG.selectAll('line, path').attr('stroke', '#666');
    }
    ...
    const link = g
      ...
      .join('line')
      .attr('stroke', (d) =>
        pathNodes.has(typeof d.source === 'string' ? d.source : d.source.id) &&
        pathNodes.has(typeof d.target === 'string' ? d.target : d.target.id)
          ? '#22c55e'
          : EDGE_COLORS[d.type]
      )
      .attr('stroke-width', (d) =>
        pathNodes.has(typeof d.source === 'string' ? d.source : d.source.id) ? 3 : 2
      )
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      d3.select(svgRef.current).attr('viewBox', `0 0 ${width} ${actualHeight}`);
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-64" align="end">
              ...
              <DropdownMenuCheckboxItem
                ...
                onCheckedChange={(checked) => setFilters({ ...filters, hideOrphans: !!checked })}
              >
              ...
              <div className="px-2 py-1">
                <Select
                  ...
                  onValueChange={(value) => setFilters({ ...filters, depthLimit: parseInt(value) })}
                >
          ...
          <MiniMap
            ...
            onViewBoxChange={(x, y) => {
              ...
              svg
                .transition()
                .duration(300)
                .call(zoom.translateTo, x + viewBox.width / 2, y + viewBox.height / 2);
            }}
          />
        )}
