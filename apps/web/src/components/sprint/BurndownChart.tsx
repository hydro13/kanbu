...
export function BurndownChart({ data, totalTasks, className = '' }: BurndownChartProps) {
  ...
  const { idealPath, actualPath, xLabels, yLabels } = useMemo(() => {
    ...
    const actualPoints = data
      .map((d, i) => ({ x: xScale(i), y: d.actual !== null ? yScale(d.actual) : null, actual: d.actual }))
      .filter((p) => p.y !== null)
    const actualPath =
    ...
    // Y axis labels
    const yLabels = [0, Math.round(maxY / 4), Math.round(maxY / 2), Math.round((3 * maxY) / 4), maxY].map(
      (v) => ({
        y: yScale(v),
        label: v.toString(),
      })
