...
const apiKey_1 = require("../trpc/procedures/apiKey");
// =============================================================================
...
function getRateLimitHeaders(keyId, limit) {
    const entry = rateLimitStore.get(keyId);
...
async function authenticateApiKey(request) {
    ...
    const keyHash = (0, apiKey_1.hashApiKey)(key);
...
// Route Handlers
// =============================================================================
async function registerPublicApiRoutes(server) {
    ...
    // Tasks
    // ==========================================================================
    server.get('/api/v1/projects/:projectId/tasks', async (request, reply) => {
        const ctx = request
            .apiContext;
        const projectId = parseInt(request.params.projectId, 10);
        ...
        const hasAccess = await prisma_1.prisma.projectMember.findFirst({
    ...
    server.post('/api/v1/tasks', async (request, reply) => {
        ...
        if (!targetColumnId) {
            const firstColumn = await prisma_1.prisma.column.findFirst({
                where: { projectId },
    ...
    // Comments
    // ==========================================================================
    server.get('/api/v1/tasks/:taskId/comments', async (request, reply) => {
        const ctx = request
            .apiContext;
        const taskId = parseInt(request.params.taskId, 10);
    ...
    server.post('/api/v1/tasks/:taskId/comments', async (request, reply) => {
        const ctx = request
            .apiContext;
        const taskId = parseInt(request.params.taskId, 10);
