...
// Input Schemas
// =============================================================================
const sprintIdSchema = zod_1.z.object({
    sprintId: zod_1.z.number(),
});
const listSprintsSchema = zod_1.z.object({
...
const createSprintSchema = zod_1.z.object({
    ...
    dateStart: zod_1.z.string(), // ISO date string
    dateEnd: zod_1.z.string(), // ISO date string
});
...
const sprintTaskSchema = zod_1.z.object({
    sprintId: zod_1.z.number(),
    taskId: zod_1.z.number(),
});
const getBurndownSchema = zod_1.z.object({
    sprintId: zod_1.z.number(),
});
// =============================================================================
// Helper Functions
 ...
 */
async function calculateBurndown(prisma, sprintId) {
...
// Router
// =============================================================================
exports.sprintRouter = (0, router_1.router)({
    /**
     * List sprints for a project
     */
    list: router_1.protectedProcedure.input(listSprintsSchema).query(async ({ ctx, input }) => {
     ...
     */
    get: router_1.protectedProcedure.input(getSprintSchema).query(async ({ ctx, input }) => {
        ...
        await (0, project_1.requireProjectAccess)(ctx.user.id, sprint.projectId, 'VIEWER');
        // Calculate stats
        const taskStats = await ctx.prisma.task.groupBy({
            by: ['isActive'],
            where: { sprintId: sprint.id },
     ...
     * Create a new sprint
     */
    create: router_1.protectedProcedure.input(createSprintSchema).mutation(async ({ ctx, input }) => {
     ...
     * Update a sprint
     */
    update: router_1.protectedProcedure.input(updateSprintSchema).mutation(async ({ ctx, input }) => {
        ...
        await (0, project_1.requireProjectAccess)(ctx.user.id, sprint.projectId, 'MEMBER');
     ...
     */
    start: router_1.protectedProcedure.input(sprintIdSchema).mutation(async ({ ctx, input }) => {
        const sprint = await ctx.prisma.sprint.findUnique({
            where: { id: input.sprintId },
        });
        if (!sprint) {
     ...
     */
    complete: router_1.protectedProcedure
        .input(zod_1.z.object({
        sprintId: zod_1.z.number(),
        moveRemainingToBacklog: zod_1.z.boolean().default(true),
    }))
     ...
     * Delete a sprint
     */
    delete: router_1.protectedProcedure.input(sprintIdSchema).mutation(async ({ ctx, input }) => {
        const sprint = await ctx.prisma.sprint.findUnique({
            where: { id: input.sprintId },
        });
        if (!sprint) {
     ...
     */
    addTask: router_1.protectedProcedure.input(sprintTaskSchema).mutation(async ({ ctx, input }) => {
     ...
     */
    removeTask: router_1.protectedProcedure.input(sprintTaskSchema).mutation(async ({ ctx, input }) => {
     ...
     */
    getBurndown: router_1.protectedProcedure.input(getBurndownSchema).query(async ({ ctx, input }) => {
        const sprint = await ctx.prisma.sprint.findUnique({
            where: { id: input.sprintId },
        });
        if (!sprint) {
     ...
     */
    getActive: router_1.protectedProcedure
