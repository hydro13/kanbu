...
async function downloadGitHubImage(
  ...
  try {
    ...
    const isJwtUrl =
      url.includes('private-user-images.githubusercontent.com') && url.includes('jwt=');
 ...
 */
async function storeImage(buffer: Buffer, filename: string, mimeType: string): Promise<string> {
...
export async function processGitHubImages(
  ...
  console.log(
    '[GitHubImageService] bodyHtml provided:',
    !!bodyHtml,
    'length:',
    bodyHtml?.length || 0
  );
  ...
  for (const image of githubImages) {
    try {
      ...
      if (image.isHtml) {
        ...
        processedContent = processedContent.replace(
          new RegExp(
            `!\\[${image.alt.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\]\\(${escapedUrl}\\)`,
            'g'
          ),
          `![${image.alt}](${localUrl})`
        );
