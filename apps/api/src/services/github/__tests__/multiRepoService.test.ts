...
describe('multiRepoService', () => {
  ...
  describe('findCrossRepoReferences', () => {
    it('should find cross-repo references in text', () => {
      ...
      expect(
        refs.some((r) => r.owner === 'hydro13' && r.repo === 'kanbu' && r.number === 123)
      ).toBe(true);
      expect(refs.some((r) => r.owner === 'hydro13' && r.repo === 'genx' && r.number === 456)).toBe(
        true
      );
      expect(
        refs.some((r) => r.owner === 'org' && r.repo === 'other-repo' && r.number === 789)
      ).toBe(true);
    });
