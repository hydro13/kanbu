...
export class RetentionService {
  ...
  async applyRetentionForType(
    ...
    try {
      ...
      const sortedFiles = [...files].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
      ...
      console.log(
        `[Retention] ${type}: kept ${result.kept.length}, deleted ${result.deleted.length}`
      );
      return result;
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
   ...
   */
  private categorizeBackups(files: BackupFile[], policy: RetentionPolicy): CategorizedBackups {
    const now = new Date();
  ...
  async previewRetention(
    ...
    return {
      ...
      toDelete: sortedFiles.filter((f) => !toKeepSet.has(f.filename)).map((f) => f.filename),
    };
  }
}
