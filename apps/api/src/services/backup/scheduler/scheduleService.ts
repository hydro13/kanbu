...
export class BackupScheduleService {
   ...
   */
  async executeByType(type: 'DATABASE' | 'SOURCE', trigger: 'EXTERNAL'): Promise<ExecutionResult> {
      ...
      const updatedExecution = await prisma.backupExecution.update({
        ...
        data: {
          ...
          fileSize: result.fileSizeKB
            ? result.fileSizeKB * 1024
            : result.fileSizeMB
              ? result.fileSizeMB * 1024 * 1024
              : null,
          completedAt,
          durationMs,
      ...
      try {
        await backupNotificationService.notifyBackupResult({
          ...
          scheduleName: scheduleId
            ? (
                await prisma.backupSchedule.findUnique({
                  where: { id: scheduleId },
                  select: { name: true },
                })
              )?.name
