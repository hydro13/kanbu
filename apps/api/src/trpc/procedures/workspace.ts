...
import { generateUniqueSlug, generateInviteToken, getInviteExpiration } from '../../lib/workspace';
...
export const workspaceRouter = router({
  ...
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    return workspaces.map((workspace) => ({
      ...
      role:
        roleMap.get(workspace.id) === 'OWNER' ? 'ADMIN' : (roleMap.get(workspace.id) ?? 'VIEWER'),
   ...
   */
  get: protectedProcedure.input(workspaceIdSchema).query(async ({ ctx, input }) => {
   ...
   * Requires at least VIEWER access
   */
  getBySlug: protectedProcedure.input(workspaceSlugSchema).query(async ({ ctx, input }) => {
    const workspace = await ctx.prisma.workspace.findUnique({
   ...
   */
  delete: protectedProcedure.input(workspaceIdSchema).mutation(async ({ ctx, input }) => {
    const access = await permissionService.requireWorkspaceAccess(
      ctx.user.id,
      input.workspaceId,
      'OWNER'
   ...
   */
  getMembers: protectedProcedure.input(workspaceIdSchema).query(async ({ ctx, input }) => {
    await permissionService.requireWorkspaceAccess(ctx.user.id, input.workspaceId, 'VIEWER');
  ...
  updateMemberRole: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const oldRole = isTargetAdmin
        ? 'ADMIN'
        : targetAcl.permissions & ACL_PERMISSIONS.WRITE
          ? 'MEMBER'
          : 'VIEWER';
   ...
   * Requires ADMIN access
   */
  getInvitations: protectedProcedure.input(workspaceIdSchema).query(async ({ ctx, input }) => {
    await permissionService.requireWorkspaceAccess(ctx.user.id, input.workspaceId, 'ADMIN');
   ...
   * Requires ADMIN or OWNER access
   */
  removeLogo: protectedProcedure.input(workspaceIdSchema).mutation(async ({ ctx, input }) => {
    await permissionService.requireWorkspaceAccess(ctx.user.id, input.workspaceId, 'ADMIN');
