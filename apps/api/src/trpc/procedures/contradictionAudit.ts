...
const updateConfigSchema = z.object({
  ...
  config: z.object({
    ...
    categoryStrategies: z
      .record(z.nativeEnum(ContradictionCategory), z.nativeEnum(ResolutionStrategy))
      .optional(),
 ...
 */
async function verifyWorkspaceWriteAccess(userId: number, workspaceId: number): Promise<void> {
...
export const contradictionAuditRouter = router({
   ...
   */
  getForPage: protectedProcedure.input(pageAuditSchema).query(async ({ ctx, input }) => {
   ...
   */
  getForWorkspace: protectedProcedure.input(workspaceAuditSchema).query(async ({ ctx, input }) => {
    const userId = ctx.user!.id;
    ...
    const revertedByIds = entries.filter((e) => e.revertedBy !== null).map((e) => e.revertedBy!);
    const revertedByUsers =
      revertedByIds.length > 0
    ...
    return {
      ...
      entries: entries.map((entry) => ({
        ...
        revertedByName: entry.revertedBy ? (revertedByNameMap.get(entry.revertedBy) ?? null) : null,
      })),
      count: entries.length,
   ...
   */
  get: protectedProcedure.input(auditEntrySchema).query(async ({ ctx, input }) => {
    const userId = ctx.user!.id;
    const auditService = getContradictionAuditService(ctx.prisma);
   ...
   */
  revert: protectedProcedure.input(revertSchema).mutation(async ({ ctx, input }) => {
    const userId = ctx.user!.id;
    const auditService = getContradictionAuditService(ctx.prisma);
    ...
    const result = await auditService.revertContradictionResolution(input.auditId, userId);
   ...
   */
  canRevert: protectedProcedure.input(auditEntrySchema).query(async ({ ctx, input }) => {
    const userId = ctx.user!.id;
    const auditService = getContradictionAuditService(ctx.prisma);
   ...
   */
  getStrategy: protectedProcedure.input(getStrategySchema).query(async ({ ctx, input }) => {
    const userId = ctx.user!.id;
  ...
  updateConfig: protectedProcedure.input(updateConfigSchema).mutation(async ({ ctx, input }) => {
    ...
    await auditService.updateWorkspaceResolutionConfig(input.workspaceId, input.config);
