...
export const wikiCommunityRouter = router({
   ...
   */
  detect: protectedProcedure.input(detectCommunitiesSchema).mutation(async ({ ctx, input }) => {
    try {
   ...
   */
  list: protectedProcedure.input(getCommunitiesSchema).query(async ({ ctx, input }) => {
    try {
      const clusterService = getWikiClusterService(ctx.prisma);
   ...
   */
  get: protectedProcedure.input(getCommunitySchema).query(async ({ ctx, input }) => {
    try {
      const clusterService = getWikiClusterService(ctx.prisma);
   ...
   */
  update: protectedProcedure.input(updateCommunitiesSchema).mutation(async ({ ctx, input }) => {
    try {
      const clusterService = getWikiClusterService(ctx.prisma);
