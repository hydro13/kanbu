...
export const aiProviderRouter = router({
   ...
   */
  list: adminProcedure.input(listProvidersSchema).query(async ({ ctx, input }) => {
    const { scope, workspaceId, projectId, providerType, isActive, capability, limit, offset } =
      input;
    ...
    if (!isDomainAdmin && scope === 'all') {
      ...
      where.OR = [{ isGlobal: true }, { workspaceId: { in: workspaceIds } }];
    }
    ...
    const [providers, total] = await Promise.all([
      ctx.prisma.aiProviderConfig.findMany({
        ...
        orderBy: [{ isGlobal: 'desc' }, { priority: 'desc' }, { name: 'asc' }],
        take: limit,
        skip: offset,
      }),
   ...
   */
  testConnection: adminProcedure.input(testConnectionSchema).mutation(async ({ ctx, input }) => {
   ...
   */
  getModels: adminProcedure.input(z.object({ id: z.number() })).query(async ({ ctx, input }) => {
    const provider = await ctx.prisma.aiProviderConfig.findUnique({
      where: { id: input.id },
    });
