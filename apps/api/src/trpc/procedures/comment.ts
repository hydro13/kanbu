...
export const commentRouter = router({
   ...
   * Requires at least VIEWER access
   */
  get: protectedProcedure.input(commentIdSchema).query(async ({ ctx, input }) => {
  ...
  update: protectedProcedure.input(updateCommentSchema).mutation(async ({ ctx, input }) => {
    ...
    if (commentInfo) {
      ...
      await auditService.logCommentEvent({
        ...
        targetName: taskInfo
          ? `${taskInfo.reference}: ${taskInfo.title}`
          : `Task #${commentInfo.taskId}`,
        userId: ctx.user.id,
        workspaceId: project?.workspaceId,
        changes: {
          contentPreview: input.content.substring(0, 100),
   ...
   */
  delete: protectedProcedure.input(commentIdSchema).mutation(async ({ ctx, input }) => {
    const { projectId, userId } = await getCommentInfo(ctx.prisma, input.commentId);
