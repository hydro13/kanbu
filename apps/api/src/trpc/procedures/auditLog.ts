...
export const auditLogRouter = router({
   ...
   */
  list: adminProcedure.input(listAuditLogsSchema).query(async ({ ctx, input }) => {
    const {
      category,
      action,
      resourceType,
   ...
   */
  get: adminProcedure.input(getAuditLogSchema).query(async ({ ctx, input }) => {
    const userScope = await scopeService.getUserScope(ctx.user!.id);
   ...
   */
  getStats: adminProcedure.input(getAuditStatsSchema).query(async ({ ctx, input }) => {
    ...
    if (!userScope.isDomainAdmin) {
      ...
      where.OR = [{ workspaceId: { in: userScope.workspaceIds } }, { workspaceId: null }];
    }
   ...
   */
  export: adminProcedure.input(exportAuditLogsSchema).mutation(async ({ ctx, input }) => {
