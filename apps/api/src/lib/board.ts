 ...
 * @throws TRPCError NOT_FOUND if column doesn't exist
 */
export async function checkWIPLimit(columnId: number): Promise<WIPValidation> {
  const column = await prisma.column.findUnique({
    where: { id: columnId },
    select: {
      id: true,
 ...
 */
export function calculateNewPositions<T extends PositionedItem>(
  items: T[],
  itemId: number,
  newPosition: number
 ...
 */
export async function applyColumnPositions(
  projectId: number,
 ...
 * @param updates - Array of {id, position} updates
 */
export async function applySwimlanePositions(
  projectId: number,
  updates: Array<{ id: number; position: number }>
): Promise<void> {
  await prisma.$transaction(
 ...
 */
export async function getNextColumnPosition(projectId: number): Promise<number> {
 ...
 * @returns Next position number
 */
export async function getNextSwimlanePosition(projectId: number): Promise<number> {
  const maxSwimlane = await prisma.swimlane.findFirst({
    where: { projectId },
    orderBy: { position: 'desc' },
    select: { position: true },
  });
