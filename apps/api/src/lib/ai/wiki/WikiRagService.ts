...
export class WikiRagService {
  ...
  async askWiki(
    ...
    if (retrievedContext.length === 0) {
      return {
        answer: 'Ik kon geen relevante informatie vinden in de wiki om je vraag te beantwoorden. Probeer je vraag anders te formuleren of controleer of er wiki pagina\'s over dit onderwerp bestaan.',
        sources: [],
        context: [],
      }
    }
  ...
  private async retrieveContext(
    ...
    const searchResults = await this.wikiEmbeddingService.semanticSearch(
      context,
      query,
      {
        workspaceId,
    ...
    for (const result of searchResults) {
      ...
      if (totalTokens + contentTokens > options.maxTokens) {
        ...
        const truncatedContent = page.content.substring(
          0,
          remainingTokens * APPROX_CHARS_PER_TOKEN
        ) + '...'
  ...
  private formatContext(context: RagContext[]): string {
    ...
    return context
      .map((page, index) => {
        const relevance = page.score >= 0.8 ? 'zeer relevant' : page.score >= 0.6 ? 'relevant' : 'mogelijk relevant'
