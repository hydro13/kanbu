...
import { labelPropagation, buildProjectionFromEdges, getClusterStats } from './algorithms';
import {
...
export class WikiClusterService {
  ...
  private async query(cypher: string): Promise<unknown[]> {
    try {
      const result = (await this.redis.call('GRAPH.QUERY', this.graphName, cypher)) as unknown[][];
