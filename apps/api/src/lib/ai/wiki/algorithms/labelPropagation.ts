 ...
 * ```
 */
export function labelPropagation(projection: LPProjectionMap, config?: LPConfig): string[][] {
 ...
 */
function collectClusters(communityMap: Map<string, number>, minClusterSize: number): string[][] {
...
export function buildProjectionFromEdges(
  nodeUuidsOrEdges: string[] | Array<{ source: string; target: string; count?: number }>,
 ...
 */
function addOrUpdateNeighbor(neighbors: LPNeighbor[], nodeUuid: string, edgeCount: number): void {
