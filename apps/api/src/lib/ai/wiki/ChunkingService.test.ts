...
import { ChunkingService, CHUNK_CONFIG, resetChunkingService } from './ChunkingService';
...
`;
      ...
      const hasHeader = result.chunks.some((chunk) => chunk.text.includes('#'));
...
`;
      ...
      const codeChunk = result.chunks.find((chunk) => chunk.text.includes('```typescript'));
...
`;
  ...
  describe('Sentence Chunking', () => {
    it('splits on sentence boundaries', () => {
      ...
      for (let i = 0; i < 100; i++) {
        sentences.push(
          `This is sentence number ${i} with some additional content to make it longer.`
        );
      }
