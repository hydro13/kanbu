...
describe('WikiNodeEmbeddingService', () => {
  ...
  describe('generateAndStoreNodeEmbedding', () => {
    it('should generate and store embedding', async () => {
      ...
      expect(result).toBe(true);
      expect(mockWikiAiService.embed).toHaveBeenCalledWith(testContext, '[Person] Robin Waslander');
  ...
  describe('generateAndStoreBatchNodeEmbeddings', () => {
    it('should process multiple nodes', async () => {
      const result = await service.generateAndStoreBatchNodeEmbeddings(testContext, testNodes);
    ...
    it('should skip nodes without name', async () => {
      ...
      const result = await service.generateAndStoreBatchNodeEmbeddings(testContext, nodesWithEmpty);
    ...
    it('should skip unchanged nodes', async () => {
      ...
      mockQdrant.retrieve.mockImplementation((_: string, params: { ids: number[] }) => {
        return Promise.resolve(
          params.ids
            .map((numericId) => {
              const node = testNodes.find(
                (n) => (service as any).generatePointId(n.id) === numericId
              );
              if (node) {
                return {
                  id: numericId,
  ...
  describe('checkNodeEmbeddingStatus', () => {
    ...
    it('should return needsUpdate=true for changed name', async () => {
      const pointId = (service as any).generatePointId('node-1');
      mockQdrant.retrieve.mockResolvedValue([{ id: pointId, payload: { nameHash: 'old-hash' } }]);
  ...
  describe('findSimilarEntities', () => {
    ...
    it('should return matching entities', async () => {
      const results = await service.findSimilarEntities(testContext, 'Robin', { limit: 10 });
    ...
    it('should respect score threshold', async () => {
      await service.findSimilarEntities(testContext, 'Robin', { threshold: 0.9 });
    ...
    it('should filter by node type when specified', async () => {
      await service.findSimilarEntities(testContext, 'Robin', { nodeType: 'Person' });

      expect(mockQdrant.search).toHaveBeenCalledWith(
        'kanbu_node_embeddings',
        expect.objectContaining({
          filter: expect.objectContaining({
            must: expect.arrayContaining([{ key: 'nodeType', match: { value: 'Person' } }]),
          }),
        })
      );
    });
    ...
    it('should filter by groupId when specified', async () => {
      await service.findSimilarEntities(testContext, 'Robin', { groupId: 'wiki-ws-1' });

      expect(mockQdrant.search).toHaveBeenCalledWith(
        'kanbu_node_embeddings',
        expect.objectContaining({
          filter: expect.objectContaining({
            must: expect.arrayContaining([{ key: 'groupId', match: { value: 'wiki-ws-1' } }]),
          }),
        })
      );
    });
    ...
    it('should exclude specified node from results', async () => {
      ...
      const results = await service.findSimilarEntities(testContext, 'Robin', {
        excludeNodeId: 'node-1-person-robin',
        limit: 10,
      });
  ...
  describe('findByNormalizedName', () => {
    it('should find exact matches', async () => {
      ...
      const results = await service.findByNormalizedName(testContext, 'robin waslander');
    ...
    it('should filter by node type', async () => {
      await service.findByNormalizedName(testContext, 'Robin', { nodeType: 'Person' });
  ...
  describe('Entity Resolution Use Cases', () => {
    ...
    it('should find "Jan" when searching for "J. Janssen"', async () => {
      ...
      const results = await service.findSimilarEntities(testContext, 'J. Janssen', {
        nodeType: 'Person',
        threshold: 0.8,
      });
    ...
    it('should find "Robin Waslander" with partial name', async () => {
      ...
      const results = await service.findSimilarEntities(testContext, 'Robin', { threshold: 0.85 });
    ...
    it('should find similar concepts', async () => {
      ...
      const results = await service.findSimilarEntities(testContext, 'Auth', {
        nodeType: 'Concept',
        threshold: 0.85,
      });
