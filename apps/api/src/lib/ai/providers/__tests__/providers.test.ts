...
import { ProviderError, DEFAULT_MODELS, DEFAULT_URLS, EMBEDDING_DIMENSIONS } from '../types';
...
describe('OpenAiProvider', () => {
  ...
  describe('listModels', () => {
    it('should return list of models', async () => {
      mockFetch.mockResolvedValueOnce(
        createMockModelsResponse(['gpt-4o-mini', 'text-embedding-3-small', 'gpt-4o'])
      );
    ...
    it('should filter models by capability', async () => {
      mockFetch.mockResolvedValueOnce(
        createMockModelsResponse(['gpt-4o-mini', 'text-embedding-3-small', 'llava:7b'])
      );
    ...
    it('should throw ProviderError on failure', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({ error: 'Server error' }, 500, false));
  ...
  describe('getDimensions', () => {
    it('should return correct dimensions for default model', () => {
      expect(provider.getDimensions()).toBe(EMBEDDING_DIMENSIONS['text-embedding-3-small']);
    });
    ...
    it('should return correct dimensions for custom model', () => {
      ...
      expect(customProvider.getDimensions()).toBe(EMBEDDING_DIMENSIONS['text-embedding-3-large']);
    });
  });
  ...
  describe('chat', () => {
    ...
    it('should throw ProviderError on rate limit', async () => {
      mockFetch.mockResolvedValueOnce(createMockResponse({ error: 'Rate limited' }, 429, false));
  ...
  describe('summarize', () => {
    it('should return summarized text', async () => {
      mockFetch.mockResolvedValueOnce(createMockChatResponse('This is a summary.'));
  ...
  describe('extractEntities', () => {
    it('should return extracted entities', async () => {
      mockFetch.mockResolvedValueOnce(
        createMockChatResponse('[{"name": "John", "type": "person", "confidence": 0.9}]')
      );

      const entities = await provider.extractEntities('John is a developer', [
        'person',
        'organization',
      ]);
    ...
    it('should return empty array on invalid JSON response', async () => {
      mockFetch.mockResolvedValueOnce(createMockChatResponse('No entities found.'));

      const entities = await provider.extractEntities('No entities', ['person']);
  ...
  describe('describeImage', () => {
    it('should return image description', async () => {
      mockFetch.mockResolvedValueOnce(createMockChatResponse('A beautiful sunset over the ocean.'));
...
describe('Model Categorization', () => {
  ...
  it('should categorize embedding models correctly', async () => {
    mockFetch.mockResolvedValueOnce(
      createMockModelsResponse(['text-embedding-3-small', 'nomic-embed-text', 'bge-large-en'])
    );
  ...
  it('should categorize reasoning models correctly', async () => {
    mockFetch.mockResolvedValueOnce(
      createMockModelsResponse(['gpt-4o-mini', 'llama3.2:8b', 'mistral:7b', 'qwen3:8b'])
    );
  ...
  it('should mark unknown models as unknown', async () => {
    mockFetch.mockResolvedValueOnce(createMockModelsResponse(['random-model-name']));
