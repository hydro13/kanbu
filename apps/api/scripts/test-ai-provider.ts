...
async function testProviders() {
  ...
  for (const config of providers) {
    ...
    console.log(
      `   Scope: ${config.isGlobal ? 'Global' : config.workspaceId ? `Workspace #${config.workspaceId}` : `Project #${config.projectId}`}`
    );
      ...
      if (result.success) {
        ...
        if (config.capabilities.includes('EMBEDDING')) {
          ...
          try {
            ...
            console.log(
              `   ❌ Embedding failed: ${err instanceof Error ? err.message : 'Unknown error'}`
            );
          }
        }
        ...
        if (config.capabilities.includes('REASONING')) {
          ...
          try {
            ...
            console.log(
              `   ✅ Reasoning successful: "${response.slice(0, 100)}${response.length > 100 ? '...' : ''}"`
            );
          } catch (err) {
            console.log(
